...
@Suite.SuiteClasses({
    ...
    TestsALL.ExtraCreditFileOpenSave.class,
...
public class TestsALL
{
    ...
    public static class A_Checkpoint1
    {
        ...
        public void testLongShortStringCell()
        {
            ...
            loc = new SpreadsheetLocation("a4");
            assertEquals("SpreadsheetLocation column", loc.getCol(), 0);
            assertEquals("SpreadsheetLocation row", loc.getRow(), 3);
        }
        ...
        public void testProcessCommandNonliteralEmpty()
        {
            ...
            if (!output.equals("") &&
                !output.toUpperCase().contains("ERROR"))
            {
    ...
    public static class A_Checkpoint2
    {
        ...
        {
            Helper helper = new Helper();
            String a1SetResult = grid.processCommand("A1 = \"oNe\"");
            helper.setItem(0, 0, "oNe");
            assertEquals("set cell return string", "", a1SetResult);
            String gridOne = grid.processCommand("print");
            ...
            String l20SetResult = grid.processCommand("L20 = \"TWo\"");
            helper.setItem(19, 11, "TWo");
            assertEquals("set cell return string", "", l20SetResult);
            String gridTwo = grid.processCommand("print");
            assertEquals("grid from accessor with two string cells", helper.getText(), gridTwo);
            String a1ClearResult = grid.processCommand("clear A1");
            helper.setItem(0, 0, "");
            assertEquals("clear cell return string", "", a1ClearResult);
            String gridOnlyTwo = grid.processCommand("print");
            assertEquals("grid with only the second string cell", helper.getText(), gridOnlyTwo);
            String clearResult = grid.processCommand("clear");
            helper.setItem(19, 11, "");
            assertEquals("clear grid", "", clearResult);
            String gridEmpty = grid.processCommand("print");
        ...
        public void testProcessCommandSpecialStrings()
        {
            ...
            Helper helper = new Helper();
            String b7SetResult = grid.processCommand("B7 = \"" + stringSpecial1 + "\"");
            assertEquals("set cell return string", "", b7SetResult);
            helper.setItem(6, 1, stringSpecial1);
            String grid1 = grid.processCommand("print");
            assertEquals("grid with one special string", helper.getText(), grid1);
            String f13SetResult = grid.processCommand("F13 = \"" + stringSpecial2 + "\"");
            assertEquals("set cell return string", "", f13SetResult);
            helper.setItem(12, 5, stringSpecial2);
            String grid2 = grid.processCommand("print");            
        ...
        public void testProcessCommandMoreSpecialStrings()
        {
            ...
            for (int col = 0; col < specialStrings.length; col++)
            {
                for (int row = 5; row < 20; row += 10)
                {
                    ...
                    String cellSetResult = grid.processCommand(cellName + " = \"" + specialStrings[col] + "\"");
                    assertEquals("set cell return string", "", cellSetResult);
                    String sheet = grid.processCommand("print");
    ...
    public static class A_Final
    {
        ...
        public void testPercentCell()
        {
            ...
            assertEquals("percent cell text", "11%", dateCell.abbreviatedCellText().trim());
            assertEquals("percent inspection text", percent, dateCell.fullCellText());
        }
        ...
        public void testBasicRealCell()
        {
            ...
            real = "3.14159265358979323846264338327";
            grid.processCommand("C12 = " + real);
            realCell = grid.getCell(new TestLocation(11, 2));
            assertEquals("real cell text", Helper.format(real), realCell.abbreviatedCellText());
            assertEquals("real inspection text", real, realCell.fullCellText());
        }
        ...
        public void testProcessCommand()
        {
            ...
            String a1SetResult = grid.processCommand("A1 = 1.021822%");
            helper.setItem(0, 0, "1%");
            assertEquals("set cell return string", "", a1SetResult);
            String first = grid.processCommand("print");
            assertEquals("grid with percent", helper.getText(), first);

            String b2SetResult = grid.processCommand("B2 = -5");
            helper.setItem(1, 1, "-5.0");
            assertEquals("set cell return string", "", b2SetResult);
            String second = grid.processCommand("print");
            assertEquals("grid with percent and number", helper.getText(), second);

            String c3SetResult = grid.processCommand("C3 = 2.718");
            helper.setItem(2, 2, "2.718");
            assertEquals("set cell return string", "", c3SetResult);
            String third = grid.processCommand("print");
            assertEquals("grid with percent and two numbers", helper.getText(), third);
            
            String d4SetResult = grid.processCommand("D4 = 0");
            helper.setItem(3, 3, "0.0");
            assertEquals("set cell return string", "", d4SetResult);
            String fourth = grid.processCommand("print");
            assertEquals("grid with percent and three numbers", helper.getText(), fourth);
        }
        ...
        public void testRealCellFormat()
        {
            ...
            for (int col = 0; col < realsEntered.length; col++)
            {
                for (int row = 6; row < 20; row += 10)
                {
                    ...
                    String setResult = grid.processCommand(cellName + " = " + realsEntered[col]);
                    helper.setItem(row,  col, realsFormatted[col]);
                    assertEquals("set cell return string", "", setResult);
                    String sheet = grid.processCommand("print");
        ...
        public void testRealCellTruncation()
        {
            ...
            assertEquals("real precise percent inspection", precisePerc, precisePerCell.fullCellText());
        }        
    }

    public static class ExtraCreditFileOpenSave
    {
        ...
        public void testSaveFormat()
        {
            ...
            assertListContains(contents, "A1,PercentCell,23.521822%");
    ...
    public static class B_Final
    {
        ...
        public void testProcessCommand()
        {
            ...
            assertEquals("set cell return string", "", actual);
            String gridText = grid.processCommand("print");
            assertEquals("grid", helper.getText(), gridText);
            String inspected = grid.processCommand("F4");
            assertEquals("inspected", "( F2 + F3 )", inspected);
            String updated = grid.processCommand("F3 = 11.5");
            assertEquals("set cell return string", "", updated);
            helper.setItem(2, 5, "11.5");
            helper.setItem(3, 5, "12.5");
            helper.setItem(4, 5, "24.0");
            gridText = grid.processCommand("print");
            assertEquals("updated grid", helper.getText(), gridText);
        ...
        public void testProcessCommandWithFunctions()
        {
            ...
            assertEquals("set cell return string", "", first);
            String gridText = grid.processCommand("print");
            assertEquals("grid with sum", helper.getText(), gridText);
            String second = grid.processCommand("G9 = ( avg A1-E4 )");
            helper.setItem(8, 6, "3.0");
            assertEquals("set cell return string", "", second);
            gridText = grid.processCommand("print");
            assertEquals("grid with sum and avg", helper.getText(), gridText);
        ...
        public void testAvgSingleNontrivial()
        {
            ...
            grid.processCommand("b1 = ( avG A2-a2 )");
            Cell cell = grid.getCell(new TestLocation(0, 1));
            String temp = cell.abbreviatedCellText();
            assertEquals("avg single cell", Helper.format("-27.0"), cell.abbreviatedCellText());
        }
    ...
    */
    ...
    public static class C_Final
    {
        ...
        public void testComparableRealMixed()
        {
            ...
            // If any of these CRASH (RED), then you did not properly implement the
            // Comparable interface on one or more of your textExcel.RealCell subclasses
            Comparable<Object> comparableA1 = (Comparable<Object>) a1;
    ...
    */
    ...
    public static class ExtraCreditCommandErrors
    {
        ...
        {
            String set = grid.processCommand("A1 = \"thrang\"");
            assertEquals("set result is empty", "", set);
            String before = grid.processCommand("print");
            String error = grid.processCommand("lesnerize");
            String after = grid.getGridText();
            assertTrue("error message starts with ERROR: ", error.toUpperCase().startsWith("ERROR: "));
        ...
        {
            String set = grid.processCommand("A1 = \"hello\"");
            assertEquals("set result is empty", "", set);
            String before = grid.processCommand("print");
            ...
            String after = grid.getGridText();
            assertTrue("error1 message starts with ERROR: ", error1.toUpperCase().startsWith("ERROR: "));
            assertTrue("error2 message starts with ERROR: ", error2.toUpperCase().startsWith("ERROR: "));
            assertTrue("error3 message starts with ERROR: ", error3.toUpperCase().startsWith("ERROR: "));
            assertTrue("error4 message starts with ERROR: ", error4.toUpperCase().startsWith("ERROR: "));
            assertEquals("grid contents unchanged", before, after);
        }
        ...
        public void testInvalidConstants()
        {
            ...
            assertTrue("error3 message starts with ERROR: ", error3.toUpperCase().startsWith("ERROR: "));
            assertTrue("error4 message starts with ERROR: ", error4.toUpperCase().startsWith("ERROR: "));
            assertTrue("error5 message starts with ERROR: ", error5.toUpperCase().startsWith("ERROR: "));
        ...
        public void testInvalidFormulaAssignment()
        {
            ...
            String set = grid.processCommand("A2 = 2");
            assertEquals("set result is empty", "", set);
            String before = grid.processCommand("print");
            ...
            assertTrue("error4 message starts with ERROR: ", error4.toUpperCase().startsWith("ERROR: "));
            assertTrue("error5 message starts with ERROR: ", error5.toUpperCase().startsWith("ERROR: "));
            assertTrue("error6 message starts with ERROR: ", error6.toUpperCase().startsWith("ERROR: "));
            assertEquals("grid contents unchanged", before, after);
        }
        ...
        public void testWhitespaceTolerance()
        {
            ...
            grid.processCommand("clear    A1");
            String clear = grid.processCommand("clear");
            assertEquals("clear result is empty", "", clear);
            String after = grid.processCommand("print");
    ...
    public static class ExtraCreditCommandHistory
    {
        ...
        {
            String historyStart = grid.processCommand("history enable " + historySize);
        ...
        {
            String historyStop = grid.processCommand("history disable");
    ...
    public static class ExtraCreditEvaluationErrors
    {
        ...
        public void testSimpleTypeErrors()
        {
            ...
            String illegalFormula = grid.processCommand("A1 = 11/20/2013");
            if (illegalFormula.toUpperCase().contains("ERROR"))
            {
                // OK to reject when setting the cell
            }
            else if (illegalFormula.equals(""))
            {
                assertEvalError(1, 0, formula, "date ref error");                
            }
            else
            {
                assertTrue("invalid value not recognized", false);
            }
